-- =====================================================
-- PropGenius AI — Initial Schema
-- Migration: 20260101000000_initial_schema
-- This captures the baseline schema already applied
...
-- RLS Policies
CREATE POLICY IF NOT EXISTS "Users can view own org"
  ON public.organizations FOR SELECT
  USING (id = public.get_user_org_id());

CREATE POLICY IF NOT EXISTS "Owners can update own org"
...
CREATE POLICY IF NOT EXISTS "Users can view org listings"
  ON public.listings FOR SELECT
  USING (organization_id = public.get_user_org_id());

CREATE POLICY IF NOT EXISTS "Users can create org listings"
  ON public.listings FOR INSERT
  WITH CHECK (organization_id = public.get_user_org_id());

CREATE POLICY IF NOT EXISTS "Users can update org listings"
  ON public.listings FOR UPDATE
  USING (organization_id = public.get_user_org_id());

CREATE POLICY IF NOT EXISTS "Users can delete org listings"
  ON public.listings FOR DELETE
  USING (organization_id = public.get_user_org_id());

CREATE POLICY IF NOT EXISTS "Users can view org leads"
  ON public.leads FOR SELECT
  USING (organization_id = public.get_user_org_id());

CREATE POLICY IF NOT EXISTS "Users can create org leads"
  ON public.leads FOR INSERT
  WITH CHECK (organization_id = public.get_user_org_id());

CREATE POLICY IF NOT EXISTS "Users can update org leads"
  ON public.leads FOR UPDATE
  USING (organization_id = public.get_user_org_id());

CREATE POLICY IF NOT EXISTS "Users can delete org leads"
  ON public.leads FOR DELETE
  USING (organization_id = public.get_user_org_id());

CREATE POLICY IF NOT EXISTS "Users can view org lead activities"
  ON public.lead_activities FOR SELECT
  USING (lead_id IN (SELECT id FROM public.leads WHERE organization_id = public.get_user_org_id()));

CREATE POLICY IF NOT EXISTS "Users can create lead activities"
  ON public.lead_activities FOR INSERT
  WITH CHECK (lead_id IN (SELECT id FROM public.leads WHERE organization_id = public.get_user_org_id()));

CREATE POLICY IF NOT EXISTS "Users can view org messages"
  ON public.whatsapp_messages FOR SELECT
  USING (organization_id = public.get_user_org_id());

CREATE POLICY IF NOT EXISTS "Users can create org messages"
  ON public.whatsapp_messages FOR INSERT
  WITH CHECK (organization_id = public.get_user_org_id());

CREATE POLICY IF NOT EXISTS "Users can view org subscriptions"
  ON public.subscriptions FOR SELECT
  USING (organization_id = public.get_user_org_id());

CREATE POLICY IF NOT EXISTS "Users can create org subscriptions"
  ON public.subscriptions FOR INSERT
  WITH CHECK (organization_id = public.get_user_org_id());

CREATE POLICY IF NOT EXISTS "Users can update org subscriptions"
...
BEGIN
  ...
  VALUES (org_id, org_name, org_slug || '-' || substr(org_id::text, 1, 8), 'free', 5, 50, 1);
...
CREATE POLICY IF NOT EXISTS "Allow owners to delete"
